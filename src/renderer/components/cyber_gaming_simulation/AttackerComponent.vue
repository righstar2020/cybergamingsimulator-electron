            ...
            <div class="tab-content-box" id="attacker-select-tab">
              <div class="attacker-statistic-info">
                  <el-row >
                    ...
                    <el-col :span="8">
                      <el-statistic title="执行操作数" :value="headerDigitalData['attackerOperationNum']['number']" />
                    </el-col>
                    <el-col :span="8">
                      <el-statistic title="攻击成功数" :value="headerDigitalData['attackerOperationNum']['number']" />
                    </el-col>
                 </el-row>
              </div>
      ...
      <el-dialog v-model="historyOperationDetailVisible" 
       ...
       >
          ...
          <div class="dialog-footer">
            ...
            <el-button type="primary" @click="openReportWindow">
              导出报告
            </el-button>
          </div>
        </template>
...
export default {
  ...
  methods: {
    ...
    runAttackerOperationsLink(){
      let currentOperationTaskList = deepCopy(this.currentAttackerSelectedOperationLinkData)
      if(currentOperationTaskList.length==0){
        this.$message.info("无可执行的操作")
        return
      }
      //this.$store.commit('addAttackerHistoryOperation',currentOperationTaskList)
      // this.attackerOperationsHistory.concat(currentOperationTaskList)
      this.currentAttackerSelectedOperationLinkData=[]
      //远端执行操作链
      proccess_operation_link_to_task('attacker',currentOperationTaskList)
